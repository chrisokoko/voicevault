...
Generate just the title, nothing else."""
    ...
    def add_audio_file_to_properties(self, page_id: str, audio_file_path: str) -> bool:
        """
        Upload audio file to Notion storage and add to page properties
        """
        try:
            ...
            if not upload_id:
                logger.error(f"Failed to upload {filename} to Notion storage")
                return False
            ...
            logger.info(f"Successfully added audio file to page properties: {filename}")
            return True
            
        except Exception as e:
            logger.error(f"Error adding audio file to page properties: {e}")
            return False
